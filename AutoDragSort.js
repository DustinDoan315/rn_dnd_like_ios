...
export default class AutoDragSortableView extends Component {
  constructor(props) {
    ...
    const height = Math.ceil(dataSource.length / rowNum) * itemHeight;
    // Height of the ScrollView
    const heightScrollView =
      ((dataSource.length + 1) * itemHeight) /
      (props.parentWidth / itemWidth - 1);
    ...
    return (
      <ScrollView
        ...
        <View
          ...
          <View onLayout={this.onLayout}>{this._renderItemView()}</View>
        </View>
      </ScrollView>
    );
  }
      ...
      return (
        <Animated.View
          ...
          <TouchableOpacity
            ...
            onPressOut={() => this.onPressOut(this.props)}
            onLongPress={() => {
